...
-- Tab and C-i to jump is the same key so noremap ctrl i to some other key first to preserve functionality
vim.keymap.set('n', '<C-y>', '<C-i>', { noremap = true }) -- jump forward , C-o is jump backward
vim.keymap.set('n', '<TAB>', '>>')
vim.keymap.set('n', '<S-TAB>', '<<')
vim.keymap.set('v', '<TAB>', '>gv')
vim.keymap.set('v', '<S-TAB>', '<gv')
vim.keymap.set('n', '<leader><leader>', '<C-6>') -- switch buffer
vim.keymap.set('n', '<leader>ww', ':set wrap!<CR>', { noremap = true, silent = true }) -- toggle line wrap
